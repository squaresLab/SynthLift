...
(** Make a Synth_stmt containing closures into a
    concrete stmt by substituting dummy values *)
let mk_cmp = Synth_stmt.fill_dummy

(** Utility to store a given frequency. So I can use this with store and
    deserialize *)
let store_with_frequency stmt (s : Synth_stmt.t) n db =
  Hashtbl.add db stmt (s,n)

(** Synth_stmts internally contain Destructs which are
    functional values and cannot be compared. For comparison purposes
    we populate the structures with dummy values. (_1,...) for vars,
    (0x1, 0x2,...) for ints, which is what Destruct.fill does *)
...
let serialize ~fname:filename (db : t) : unit =
  let add db ch =
    Hashtbl.iter (fun stmt {freq = i;_} ->
        ...
        Sexp.output_hum ch line (** TODO: change to output after debug *)
      ) db in
  let open Core_kernel.Std in
...
let pp db =
  let open Core_kernel.Std in
  (* What is the distribution of ast structure, with percentage? *)
  Format.printf "------------------------------------------------@.";
